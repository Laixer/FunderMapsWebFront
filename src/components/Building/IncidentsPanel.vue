...
const fieldsWithData = computed(() => {
  ...
  const fieldsConfig = applyContextToFieldDataConfigs({
    ...
    configs: [
      ...
      new FieldDataConfig({ name: 'name', default: 'Afgeschermd' }),
      new FieldDataConfig({ name: 'phoneNumber', default: 'Afgeschermd' }),
      new FieldDataConfig({ name: 'email', default: 'Afgeschermd' }),
  ...
  <Panel 
    ...
    >
      <div>
        ...
        <nav
          v-if="maxListIndex > 1"
          class="mb-2 flex items-center justify-between text-sm text-grey-700">
          <button
          ...
          <span>{{ selectedListIndex + 1 }} van {{ maxListIndex + 1 }}</span>
          <button
            class="button button--link p-1"
